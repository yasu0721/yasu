...
// 素材の読み込みから最終案の保存までの一連の処理。
// CLI(automation/generate.js)からも、ローカルWeb画面(automation/server.js)
// からも、この同じ関数を呼び出す(UI専用ロジックにしないため)。
//
// onProgress(message) は進捗メッセージを1行ずつ通知するためのコールバック。
// 戻り値: { status, runId, finalFile, success, failed, message }
//   status: 'no_material' | 'parse_failed' | 'no_candidates' | 'editor_failed' | 'router_failed' | 'success'
async function runPipeline({ onProgress = () => {} } = {}) {
  ...
  try {
    ...
    return { status: 'read_failed', message: `input.txt を読み込めませんでした: ${err.message}` };
  }

  if (material.length === 0) {
    onProgress('input.txt に素材がありません。');
    writeLog('generate', '入力なし');
    return { status: 'no_material', message: 'input.txt に素材がありません。' };
  }
  ...
  onProgress(`[1/4] 素材を読み込みました(run_id: ${runId})`);
  ...
  try {
    ...
    return { status: 'router_failed', runId, message: `振り分けGPTの実行に失敗しました: ${err.message}` };
  }
  saveOutput(
    runDir,
    'router.txt',
  ...
  onProgress(`[3/4] 専門GPT ${targetIds.length}体が分析中です(${targetIds.join(', ')})`);
  if (skippedIds.length > 0) {
    onProgress(`※ 上限(${MAX_SPECIALISTS}体)を超えたため見送り: ${skippedIds.join(', ')}`);
  ...
  for (const id of targetIds) {
    try {
      ...
      onProgress(`${id} の回答取得に失敗しました(処理は続けます)`);
    }
  }

  if (results.success.length === 0) {
    const msg = '専門GPTの回答が1件も得られなかったため、編集長GPTへは送らず停止します。';
    onProgress(msg);
    writeLog('generate', '停止(候補0件)', `run_id=${runId}`);
    return { status: 'no_candidates', runId, success: results.success, failed: results.failed, message: msg };
  }
  if (results.success.length === 1) {
    onProgress('候補が1件のみですが続行します。');
    writeLog('generate', '候補1件のみで続行', `run_id=${runId}`);
  }

  // --- 編集長GPT ---
  onProgress('[4/4] 編集長GPTが最終候補を選んでいます');
  ...
  try {
    ...
    return {
      ...
      message: `編集長GPTの実行に失敗しました: ${err.message}(専門GPTの回答は保存済みです: ${runDir})`,
    };
  }
  ...
  onProgress('完成しました。');
  onProgress(`最終結果: ${finalFile}`);
  if (bestCount > 0) {
    onProgress(`個別案も保存しました(best_1.txt など、${bestCount}件)`);
  }
  ...
  return {
    ...
    runId,
    finalFile,
    finalText: editorResult.text,
    success: results.success,
    failed: results.failed,
  };
}

module.exports = { runPipeline, INPUT_FILE };
